...
#define BACKEND_PORT 31574  // port number used by the backend server
#define FAILURE -1
...
unordered_map<string, unordered_set<string>> deptStudentIDs;
unordered_map<string, vector<int>> studentGrades;
string deptList;
unordered_map<string, string> studentGPA;
unordered_map<string, vector<double>> deptGPAs;
unordered_map<string, string> deptStats;
...
int main() {
   ...
   cout << "Server B is up and running using UDP on port " << BACKEND_PORT << endl;
   server_boot_up();
   ...
   while (true) {
      ...
      if (message_length == 0) {
         ...
         cout << "Server B has received request for Student " << studentID << " in " <<
              dept << "." << endl;

         string msgToMain;
         if (! deptStudentIDs[dept].count(studentID)) {
            msgToMain = "Student " + studentID + " : not found.\n";
            cout << endl << "Server B has sent \"Student " << studentID << " not found\" to Main Server." << endl;
         }
         else {
            //prepare the message to main server
            msgToMain= studentGPA[studentID] + "\t";
            msgToMain += studentRank(dept, studentID) + "\t" + deptStats[dept];
            cout << "Server B calculated following academic statistics for Student " << studentID <<
                 " in " << dept << endl;
            ...
            cout << endl << "Server B has sent results to Main Server." << endl;
